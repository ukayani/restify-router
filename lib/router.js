...
function mergeWithGroupPrefix(router, opts) {
  ...
  var path = router.groupPrefix + opts.path;

  // Avoid extra slash
  opts.path = (path.length > 1 && path.substr(-1) === '/')
...
var methods = ['get', 'post', 'put', 'del', 'patch', 'head', 'opts', 'group'];
...
methods.forEach(function (method) {
  Router.prototype[method] = function (options) {
    ...
    if (method === 'group') {
      this.groupPrefix = this.groupPrefix + opts.path;

      var handlers = getHandlersFromArgs(arguments, 1);
      var groupHandler = handlers.pop();
